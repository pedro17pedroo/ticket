...
const WatchersField = ({ watchers = [], onChange, placeholder = "Digite emails para notificar..." }) => {
  ...
  const addWatcher = () => {
    ...
    // Evitar duplicatas
    const existingEmails = watchers.map(w => typeof w === 'string' ? w : w.email);
  ...
  const removeWatcher = (emailToRemove) => {
    onChange(watchers.filter(w => {
      const watcherEmail = typeof w === 'string' ? w : w.email;
      return watcherEmail !== emailToRemove;
    }));
  };
  ...
  const watcherEmails = watchers.map(w => typeof w === 'string' ? w : w.email);
