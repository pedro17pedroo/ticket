...
const CreatePlan = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name || !formData.displayName) {
      showError('Nome e Nome de Exibição são obrigatórios');
      return;
    }
    try {
      setLoading(true);
      ...
      showError(error.response?.data?.error || 'Erro ao criar plano');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      <div className="mb-6 flex items-center gap-4">
        <Button variant="ghost" icon={<ArrowLeft className="w-4 h-4" />} onClick={() => navigate('/plans')}>Voltar</Button>
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Novo Plano</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">Cadastre um novo plano de assinatura</p>
        </div>
      </div>
          ...
          <Card className="lg:col-span-2">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <Input label="ID do Plano (slug)" name="name" value={formData.name} onChange={handleChange} required placeholder="starter, professional, enterprise" />
                <Input label="Nome de Exibição" name="displayName" value={formData.displayName} onChange={handleChange} required placeholder="Starter, Professional, Enterprise" />
              </div>
              <Input label="Descrição" name="description" value={formData.description} onChange={handleChange} placeholder="Ideal para pequenas equipas" />
            ...
            <div className="space-y-2 mb-4">
              {formData.featuresText.map((feature, index) => (
                <div key={index} className="flex items-center gap-2 bg-gray-50 dark:bg-gray-700 p-2 rounded-lg">
                  ...
                  <button type="button" onClick={() => removeFeatureText(index)} className="text-red-500 hover:text-red-700"><X className="w-4 h-4" /></button>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-6">
            <Card>
              ...
              <div className="space-y-3">
                {[['basicTicketing', 'Tickets Básicos'], ['emailIntegration', 'Integração Email'], ['knowledgeBase', 'Base de Conhecimento'],
                  ['slaManagement', 'Gestão de SLA'], ['reporting', 'Relatórios'], ['automation', 'Automações'],
                  ['apiAccess', 'Acesso à API'], ['whiteLabel', 'White-label'], ['prioritySupport', 'Suporte Prioritário']
                ].map(([key, label]) => (
            ...
            <Card>
              <div className="space-y-3">
                <Button type="submit" className="w-full" loading={loading} icon={<Save className="w-4 h-4" />}>Criar Plano</Button>
                <Button type="button" variant="ghost" className="w-full" onClick={() => navigate('/plans')} disabled={loading}>Cancelar</Button>
              </div>
            </Card>
          </div>
        </div>
