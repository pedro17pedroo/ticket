...
const Sections = () => {
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const [sectionsRes, depsRes] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingSection) {
        await api.put(`/client/sections/${editingSection.id}`, formData)
        showSuccess('Atualizada!', 'Secção atualizada com sucesso')
      } else {
        await api.post('/client/sections', formData)
        showSuccess('Criada!', 'Secção criada com sucesso')
      }
      setShowModal(false)
      resetForm()
      loadData()
  ...
  const handleDelete = async (id) => {
    const confirmed = await confirmDelete('Desativar secção?', 'Esta ação pode ser revertida.')
    if (!confirmed) return
    try {
      await api.delete(`/client/sections/${id}`)
      showSuccess('Desativada!', 'Secção desativada com sucesso')
      loadData()
    } catch (error) {
  ...
  const resetForm = () => {
    setFormData({ name: '', description: '', code: '', departmentId: '', isActive: true })
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {sections.map((section) => (
                <tr key={section.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                  <td className="px-6 py-4 font-medium">{section.name}</td>
                  <td className="px-6 py-4">{section.department?.name || '-'}</td>
        ...
        <div className="flex items-center justify-center bg-black/50 p-4" style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', zIndex: 9999 }}>
          <div className="bg-white dark:bg-gray-800 rounded-xl max-w-lg w-full">
            <div className="flex items-center justify-between p-6 border-b">
              <h3 className="text-xl font-bold">{editingSection ? 'Editar' : 'Nova'} Secção</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <select value={formData.departmentId} onChange={(e) => setFormData({ ...formData, departmentId: e.target.value })} required className="w-full px-4 py-2 border rounded-lg dark:bg-gray-700">
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button type="submit" className="flex-1 px-4 py-2 bg-primary-600 text-white rounded-lg">{editingSection ? 'Atualizar' : 'Criar'}</button>
              </div>
            </form>
          </div>
        </div>
