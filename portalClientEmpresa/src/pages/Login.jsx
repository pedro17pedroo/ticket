...
const Login = () => {
  ...
  const handleResetPassword = async ({ token, newPassword, confirmPassword }) => {
    ...
    try {
      setRecoveryLoading(true)
      await authService.validatePasswordResetToken(recoveryEmail, token, 'client')
      await authService.resetPasswordWithToken(recoveryEmail, token, newPassword, 'client')
